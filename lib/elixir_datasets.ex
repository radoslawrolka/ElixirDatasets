defmodule ElixirDatasets do
  ...
  def load_spec(repository, opts \\ []) do
    IO.inspect({:load_spec, repository, opts},
      label: "Bumblebee.load_spec"
    )
  ...
  defp do_load_spec(repository, repo_files, module, architecture) do
    IO.inspect({:do_load_spec, repository, repo_files, module, architecture},
      label: "do_load_spec"
    )
  ...
  def load_dataset(repository, opt \\ []) do
    ...
    # {:ok, dataset} <- load_from_file(paths)
  ...
  defp get_repo_files({:hf, repository_id, opts}) do
    IO.inspect({:get_repo_files, repository_id, opts},
      label: "Bumblebee.get_repo_files"
    )
  ...
  defp download({:hf, repository_id, opts}, filename, etag) do
    ...
    IO.inspect({:download, repository_id, filename, etag, opts},
      label: "Bumblebee.download"
    )
